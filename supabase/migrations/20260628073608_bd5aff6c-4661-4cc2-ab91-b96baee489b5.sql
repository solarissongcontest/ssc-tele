CREATE OR REPLACE FUNCTION public.submit_vote(p_round_id uuid, p_username text, p_country_code text, p_entries jsonb, p_ip_hash text DEFAULT NULL::text, p_fingerprint_hash text DEFAULT NULL::text, p_device_token_hash text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Home country may be ANY Solaris country, not just one competing in the round.
  -- It only needs to exist in the global countries table.
